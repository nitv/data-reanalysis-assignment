...
I read in the .dta file, which is a file formatted for the Stata environment, I used the `read.dta` function from the `foreign` package as shown below.

```{r}
library(foreign)
d <- read.dta("D:\\Data\\Nitin\\UT Austin\\data analysis\\vww_ajps_replication_data.dta")
names(d)
head(d)
```
